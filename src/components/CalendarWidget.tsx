...
export default function CalendarWidget({ setActiveTab }: { setActiveTab: (t: string) => void }) {
  ...
  return (
    <div className="bg-white/80 dark:bg-[#0a231b]/60 backdrop-blur-md border border-emerald-900/10 dark:border-emerald-500/20 rounded-[2rem] p-6 sm:p-8 hover:border-emerald-500/30 transition-all duration-300 shadow-[0_4px_20px_rgba(4,40,24,0.04)] dark:shadow-xl">
      {/* Header */}
        ...
        {displayEvents.map((event) => {
          ...
          return (
            <div 
              key={event.id}
              onClick={() => setActiveTab('calendar')}
              className="group cursor-pointer flex flex-col justify-between p-5 rounded-2xl bg-emerald-50/50 dark:bg-[#061811]/60 border border-emerald-900/10 dark:border-white/5 hover:border-emerald-500/30 hover:bg-emerald-100/50 dark:hover:bg-[#0b281f]/70 hover:-translate-y-1 transition-all duration-200"
            >
              <div>
                <div className="flex items-center justify-between gap-2 mb-3">
                  ...
                  <span className="text-[11px] font-bold text-emerald-900/60 dark:text-slate-400 uppercase tracking-wider">
                    {event.monthName} {event.year}
                  </span>
                </div>

                <h4 className="text-base font-bold text-slate-900 dark:text-slate-100 group-hover:text-emerald-700 dark:group-hover:text-emerald-300 transition-colors line-clamp-2 leading-snug">
                  {event.title}
                </h4>
              </div>

              <div className="mt-4 pt-3 border-t border-emerald-900/10 dark:border-white/5 flex items-center justify-between text-xs text-slate-600 dark:text-slate-400">
                <span className="inline-flex items-center gap-1.5 text-slate-700 dark:text-slate-300 font-medium truncate max-w-[200px]">
                  <Clock size={12} className="text-emerald-600 dark:text-emerald-400 shrink-0" />
                  <span className="truncate">{event.displayDate}</span>
                </span>
                <ArrowRight size={14} className="text-emerald-600/70 dark:text-slate-500 group-hover:text-emerald-600 dark:group-hover:text-emerald-400 group-hover:translate-x-1 transition-all shrink-0 ml-2" />
              </div>
            </div>
          );
        })}
